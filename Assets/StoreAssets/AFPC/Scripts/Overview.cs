...
namespace AFPC {
    ...
    public class Overview {
        ...
        /// </summary>
        public virtual void Looking (Transform t) {
		    if (!camera) return;
            ...
            t.Rotate(Vector3.up * targetRotation.y * mouseSensitivity);

            verticalLookRotation += targetRotation.x * mouseSensitivity;
        ...
        public void RotateRigigbodyToLookDirection (Rigidbody rb) {
            ...
            if (!camera) return;
            if(camera.transform.rotation.eulerAngles.y - rb.rotation.y > 0.5f)
